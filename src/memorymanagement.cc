...
void* MemoryManager::malloc(size_t size) {
  ...
  // NOTE: available space needs to be (requested size + metadata) since requested size does not account for metadata of
  // MemoryChunk NOTE: "+ 1" acts like a buffer between requested size and the next MemoryChunk*; without the "+ 1", the
  // requested size would be directly followed by the metadata for the next MemoryChunk*
  ...
  if (result->size >=
    ...
    /*
      ...
      (time=0) | MEMORYDIMENSION := {[chunk1], [chunk2], [ ... unallocated space (64 KB) ... ], [chunk3], [chunk4], [
      ...
      (time=1) | result_chunk := first unallocated sector in memory space greater than requested size (8192 bytes), see
      for loop above for more details (time=1) | result_chunk->size = 65536 bytes ( the result_chunk->size ==
      unallocated space)

      (time=2) | the result_chunk->size is greater than (metadata + requested size = 8702), so partition the
      result_chunk (unallocated space) , at this point, the unallocated has no metadata (time=2) | turn the result_chunk
      (unallocated space) into a MemoryChunk (result_chunk->size + metadata) (time=2) | result_chunk := [ (metadata +
      requested size) | remaining_after_partitioning |] (time=2) | initalize metadata values (next, prev, allocated,
      size) for remaining_chunk,(e.g. remaining_chunk->size = result_chunk->size - (metadata + requested size)) (time=2)
      | result_chunk := [ (metadata + requested size)], unallocated_space := [ remaining_after_partitioning] (time=2) |
      now, the result_chunk->size = (metadata + requested size), so we are good to return the result_chunk, the
      remaining_chunk just becomes unallocated space again

      (time=3) | MEMORYDIMENSION := {[chunk1], [chunk2], [chunk5], [ ... unallocated space (55.5 KB) ... ], [chunk3],
      [chunk4], [ ... unallocated space (16 KB) ... ] } => entire memory space
