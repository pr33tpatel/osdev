...
bool Widget::ContainsCoordinate(int32_t x, int32_t y) {
  return
      // NOTE: a full screen widget would start at (0,0), so full screen widget would always return `true` to
      // ContainsCoordinate
      this->x <= x  // relative x is less than or equal to absolute x (x of coordinate is less than x of widget)
      && x < this->x + this->w   // relative x is less than absolute x + width of widget (the x position of widget +
                                 // width of widget)
...
CompositeWidget::CompositeWidget(
    Widget* parent, int32_t x, int32_t y, int32_t w, int32_t h, uint8_t r, uint8_t g, uint8_t b
)
...
void CompositeWidget::OnMouseMove(int32_t oldx, int32_t oldy, int32_t newx, int32_t newy) {
  /* FIXME: OnMouseMove code is really ugly. instead there should be functions like OnMouseEnter and OnMouseLeave which
   * are called whenever the mouse is travelling between widgets */
  int firstChild = -1;
  for (int i = 0; i < numChildren; i++)
...
void CompositeWidget::OnKeyDown(char str) {
  if (focusedChild != 0) focusedChild->OnKeyDown(str);
}


void CompositeWidget::OnKeyUp(char str) {
  if (focusedChild != 0) focusedChild->OnKeyUp(str);
}
