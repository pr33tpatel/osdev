...
namespace os {
  ...
  class MemoryManager {
    ...
    public:
      ...
      // NOTE: void* is used for malloc() because malloc should work for any data type of any size. the size is given to malloc as an argument 
      void* malloc(common::size_t size); // NOTE: returns pointer to an object of *unkown* size (size is passed in as the argument)
      void free(void* ptr);
  };
